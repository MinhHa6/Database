...
SELECT * FROM MonHoc
SELECT * FROM Ketqua
SELECT MH.TENMH,KQ.DIEM FROM MonHoc MH
INNER JOIN Ketqua KQ ON KQ.Diem = MH.MaMH
SELECT SV.HOSV+' '+SV.TENSV HOTEN,MH.TENMH FROM SINHVIEN SV
INNER JOIN  KHOA KH ON SV.MaKH= KH.MaKH
INNER JOIN MonHoc MH ON KH.MaKH = MH.MaMH
--INNER JOIN KETQUA KQ
--(SELECT MASV,MAX(DIEM) MAX_KQ FROM Ketqua KQ GROUP BY MaSV) 
